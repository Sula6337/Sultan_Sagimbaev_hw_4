...
public class Main {
    ...
    public static void chooseBossDefence() {
        ...
        int randomIndex = random.nextInt(2); //0,1,2
    ...
    public static int healHealth () {
        ...
        return randomHeal.nextInt(70);
    }
    public static void medicSuper() {
        for (int i = 0; i < heroesHealth.length; i++) {
            if (heroesHealth[i] <= 100 && heroesAttackType[i] != heroesAttackType[3] && heroesHealth[i] > 0) {
                int healHeroes = healHealth();
                heroesHealth[i] = heroesHealth[i] + healHeroes;
                System.out.println("Медик вылечил на: " + healHeroes);
                break;
            }
        }
    }
